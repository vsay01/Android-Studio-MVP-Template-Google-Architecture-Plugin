...
class ${className}Activity : BaseActivity(), ${className}Fragment.On${className}FragmentInteractionListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val ${className.capitalize()}Fragment = ${className}Fragment.newInstance()
        supportFragmentManager
                .beginTransaction()
                .replace(R.id.frame_layout_content, ${className.capitalize()}Fragment, "${className}Fragment")
                .addToBackStack(null)
                .commit()

        presenter = ${className}Presenter(this, ${className.capitalize()}Fragment)
    }
